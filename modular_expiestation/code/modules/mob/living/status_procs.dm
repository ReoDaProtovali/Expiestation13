...
/**
 * Adjust how opiated the mob currently is, and applies an opiated effect if they're
 * not currently opiated.
 *
 ...
 */
/mob/living/proc/adjust_opiated_effect(amount, down_to = 0, up_to = INFINITY)
	if(!isnum(amount))
		CRASH("adjust_drunk_effect: called with an invalid amount. (Got: [amount])")

	var/datum/status_effect/opiated/opiated = has_status_effect(/datum/status_effect/opiated)
	if(opiated)
		opiated.set_opiate_value(clamp(opiated.opiate_dose + amount, down_to, up_to))
	else if(amount > 0)
		apply_status_effect(/datum/status_effect/opiated, amount)
 ...
 */
/mob/living/proc/set_opiate_effect(set_to)
	if(!isnum(set_to) || set_to < 0)
		CRASH("set_opiate_effect: called with an invalid value. (Got: [set_to])")

	var/datum/status_effect/opiated/opiated = has_status_effect(/datum/status_effect/opiated)
	if(opiated)
		opiated.set_opiate_value(set_to)
	else if(set_to > 0)
		apply_status_effect(/datum/status_effect/opiated, set_to)

/// Helper to get the amount of drunkness the mob's currently experiencing.
/mob/living/proc/get_opiate_amount()
	var/datum/status_effect/opiated/opiated = has_status_effect(/datum/status_effect/opiated)
	return opiated?.opiate_dose || 0
