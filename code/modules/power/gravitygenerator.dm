...
/obj/machinery/gravity_generator/main/proc/shake_everyone()
	...
	for(var/mob/mobs as anything in GLOB.mob_list)
		...
		if(!istype(mob_turf))
			continue
		if(!SSmapping.are_same_zstack(T.z, mob_turf.z))
			continue
		if(isliving(mobs))
...
/obj/machinery/gravity_generator/main/hypotheticalShuttleMove(turf/newT, rotation, move_mode, obj/docking_port/mobile/moving_dock)
	. = ..()
	disable()
